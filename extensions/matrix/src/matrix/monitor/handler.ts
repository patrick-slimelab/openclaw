        ...
        ? roomConfig?.autoReply === true
          ...
          : roomConfig?.autoReply === false
            ...
            : typeof roomConfig?.requireMention === "boolean"
              ...
              : true
        : false;
      // Keyword detection for rooms with keywordMention enabled
      const keywordConfig = roomConfig?.keywordMention ?? false;
      const keywordPatterns = [/\\bscoob[a-z]*\\b/i, /\\b[a-z]*hound\\b/i];
      const hasKeyword = keywordConfig && keywordPatterns.some((pattern) => pattern.test(bodyText));
      const shouldBypassKeywordCheck =
        isRoom && shouldRequireMention && !wasMentioned && hasKeyword;
